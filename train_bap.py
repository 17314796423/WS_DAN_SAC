...
def train():
    ...
    if use_gpu:
        ...
        # 使用0.499的显存:
        tmp_tensor = torch.empty(int(total_memory * 0.90), dtype=torch.int8, device='cuda')
        # 清空该显存：
        del tmp_tensor
        torch.cuda.empty_cache()
    ...
    for e in range(config.epochs):
        ...
        }, is_best, False, config.checkpoint_path)
...
def test():
    ...
    if use_gpu:
        criterion = criterion.cuda()
    prec1, prec5 = engine.test(val_loader, net, config)
